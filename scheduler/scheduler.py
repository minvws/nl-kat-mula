...
from scheduler import (context, dispatcher, dispatchers, models, queue, queues,
                       ranker, rankers, server)
...
class Scheduler:
    ...
    def add_p_items_to_queue(self, pq: queue.PriorityQueue, p_items: List[queue.PrioritizedItem]) -> None:
        ...
        if count > 0:
            self.logger.info(
                ...
                count, pq.id, pq.id, count,
            )

    def get_p_items_for_oois(self, oois: List[models.OOI]) -> List[queue.PrioritizedItem]:
    ...
    def _populate_boefjes_queue(self) -> None:
        ...
        for org in orgs:
            try:
                latest_oois = self.ctx.services.octopoes.get_latest_objects_since(organisation_id=org.id, n=10, since=timedelta(days=1)) # FIXME: last check
            except (requests.exceptions.RetryError, requests.exceptions.ConnectionError):
        ...
        # When the queue isn't full we will add it with random objects
        for org in orgs:
            try:
